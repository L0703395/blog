...
// Update: No photo/parallax background; ornamental gradients only.
//         BASE_URL-safe audio path. Lighter bundle.
// =============================================
...
const Section: React.FC<{ title: string; subtitle?: string; children: React.ReactNode }> = ({
  ...
  return (
    ...
    >
      ...
      {subtitle && (
        <p style={{ ...serif, color: palette.accent, margin: "6px 0 14px", fontStyle: "italic" }}>{subtitle}</p>
      )}
      <div>{children}</div>
    </section>
  );
...
const WeeklyReflection: React.FC = () => {
  const [open, setOpen] = useState(false); // resets to closed each load
  return (
    <div style={{ marginTop: 6 }}>
      <button
        onClick={() => setOpen((o) => !o)}
        style={{ background: "transparent", border: "none", color: palette.accent, textDecoration: "underline", cursor: "pointer" }}
      >
        {open ? "Hide weekly notes" : "Read this week's notes"}
      </button>
      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: open ? "auto" : 0, opacity: open ? 1 : 0 }} style={{ overflow: "hidden" }}>
          ...
          <nav style={{ display: "flex", gap: 14, marginTop: 14, flexWrap: "wrap" }}>
            {["Home", "Vocabulary", "Recipes", "Poems", "Hobbies", "Traditions", "Music"].map((item) => (
              <a
                key={item}
                href="#"
                style={{ ...serif, color: palette.ink, textDecoration: "none", borderBottom: `1px solid ${palette.accent2}55`, paddingBottom: 2 }}
              >
                {item}
              </a>
            ))}
