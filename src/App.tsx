...
const hobbies = [
  {
    title: "Pressed Leaf Almanac",
    cycle: "2025-10",
    note: "Collecting, pressing, and cataloging leaves with notes.",
  },
  {
    title: "Hand-stitched Apron",
    cycle: "2025-12",
    note: "Linen apron with cross-back straps.",
  },
];
  ...
  return (
    <section
      style={{
        margin: "36px 0",
        padding: "16px 0",
        borderTop: `1px dashed ${palette.accent2}55`,
      }}
    >
      <h2 style={{ ...serif, color: palette.ink, fontSize: 28, margin: 0 }}>
        {title}
      </h2>
      {subtitle && (
        <p
...
const Card: React.FC<{ children: React.ReactNode; onClick?: () => void }> = ({
  children,
  onClick,
}) => {
  return (
    ...
    <div style={{ position: "fixed", right: 16, bottom: 16, zIndex: 50 }}>
      <button
        ...
        style={{
          ...
          borderRadius: 999,
          padding: "10px 14px",
          boxShadow: `0 4px 12px ${palette.berry}55`,
          fontSize: 14,
        }}
      >
        {open ? "✕" : "🎵"}
    ...
    >
      ...
      >
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "40px 20px 24px" }}>
          <h1 style={{ ...script, fontSize: 46, margin: 0, color: palette.ink }}>
            A Journey Within
          </h1>
          <div style={{ ...serif, fontSize: 18, color: palette.accent }}>
            Notes from Solitude
          </div>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              <span style={{ fontSize: 14, ...serif, color: palette.berry }}>
                Seasonal Greeting
              </span>
          ...
          <nav style={{ display: "flex", gap: 14, marginTop: 14, flexWrap: "wrap" }}>
            {["Home", "Vocabulary", "Recipes", "Poems", "Hobbies", "Traditions", "Music"].map(
              (item) => (
                <a
                  key={item}
                  href="#"
                  style={{
                    ...
                    color: palette.ink,
                    textDecoration: "none",
                    borderBottom: `1px solid ${palette.accent2}55`,
                    paddingBottom: 2,
                  }}
                >
                  {item}
      ...
      >
          ...
          <Card>
            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 6 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
                <div style={{ ...serif, fontSize: 24, color: palette.ink }}>
                  {dailyWord.word}
                </div>
                <div style={{ fontSize: 12, color: palette.accent2, ...mono }}>
                  {dailyWord.pos}
                </div>
              </div>
            ...
            {recipeFeed.map((r) => (
              <Card key={r.slug}>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  <div style={{ fontSize: 12, color: palette.accent2 }}>
                    Updated: {r.updated}
                  </div>
                </div>
              </Card>
        ...
        <Section title="Weekly Poem">
          <Card>
            <div style={{ ...serif }}>
              <div style={{ fontSize: 18, marginBottom: 8, color: palette.ink }}>
                {poemOfWeek.title}
              </div>
              <pre
                style={{
...
if (typeof import.meta !== "undefined" && (import.meta as any).vitest) {
  const { it, expect, describe } = (import.meta as any).vitest;

  describe("Autumn Blog — sanity", () => {
    it("palette has the expected keys", () => {
      const keys = Object.keys(palette).sort();
      expect(keys).toEqual(
        ["accent", "accent2", "berry", "bg", "gold", "ink", "leaf"].sort()
      );
    });

    it("palette values look like hex colors", () => {
      const hex = /^#([0-9A-F]{3}){1,2}$/i;
      for (const v of Object.values(palette)) expect(hex.test(String(v))).toBe(true);
    });

    it("content arrays are non-empty", () => {
